...
new class extends Component
{
    ...
    public function submit(): void
    {
        ...
        if ($this->latitude === null || $this->longitude === null) {
            $this->addError('location', __('Veuillez autoriser la geolocalisation ou preciser l\'emplacement.'));
            return;
        }
...
<div class="min-h-screen bg-gray-50 py-8 px-4">
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-2">{{ __('Signaler un nid-de-poule') }}</h1>
        <p class="text-gray-600 mb-6">{{ __('Aidez-nous a ameliorer les rues de Montreal en signalant les nids-de-poule.') }}</p>

        @if ($submitted)
            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">
                {{ __('Merci ! Votre signalement a ete transmis.') }}
            </div>
        @endif
        ...
        <form wire:submit="submit" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">{{ __('Courriel') }} *</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">{{ __('Categorie') }} *</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">{{ __('Adresse') }} *</label>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700">{{ __('Quartier') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">{{ __('Arrondissement') }}</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">{{ __('Localisation') }} *</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">{{ __('Photos') }} ({{ __('max 5') }})</label>
                ...
                @error('photos.*') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                <div wire:loading wire:target="photos" class="text-sm text-gray-500 mt-1">{{ __('Chargement...') }}</div>
            </div>
